...
目标：
1 文章高亮单词词组提取
1.5 根据标题，解析epub.txt，获得例句
2 单词词组Cambridge查询并构建词库
3 与现有词库vocabulary.data中的对比，区分重复的不重复的
前提准备:
文章橙色高亮
打开notion
保存vocabulary.data，可以在本代码中执行一次
关掉vpn
...
class Economists:
    ...
    # https://www.notion.so/94cc35f2d7ba46e0b9bdaecce97c116b?v=4be3c196c0364fa28602bda3019b40c7&pvs=4
    ...
    def __init__(self):
        ...
        self.database_id = config.get('database', 'python_query')  # 取database前边的
    ...
    def get_cambridge_origin_pronoun_voice(self,soup, flag = 1):
        ...
        for pronoun_span in pronoun_spans:
            ...
            if count == 1:
                ...
                pronounciation += '/' + res + '/'
            if len(pronounciation) != 0 and count == 2:
                break
    ...
    def run(self):
        ...
        with open("../TxtDataFiles/words.txt", 'w', encoding=self.detected_encoding) as file:
            ...
            for w in self.words:
                file.write(w + "\n")
        with open("notion上传.txt", 'w',encoding=self.detected_encoding) as file:
            file.truncate()
        with open("重复.txt", 'w',encoding=self.detected_encoding) as file:
            file.truncate()
        ...
        for i in range(len(self.words)):
            ...
            if self.words[i] not in error_words:
                if self.words[i] in self.my_dict:
                    chongfu_num += 1
                    with open("重复.txt","a", encoding='utf-8') as file:
            ...
            # if self.words[i] not in error_words:
                    with open("notion上传.txt","a",encoding='utf-8') as file:
        ...
        if chongfu_num + len(up_meaning) == len(self.words):
            input("都对应上了，可以enter")
        subprocess.run(['notepad.exe',"notion上传.txt"],check = True)
        subprocess.run(['notepad.exe',"重复.txt"],check = True)
